...
int main()
{
	...
	NewPlayer2.SetRenderChar('#');


	const int BulletCount = 1000;
	const int BulletCount2 = 1000;
	Bullet* NewBullet = new Bullet[BulletCount];
	Bullet* NewBullet2 = new Bullet[BulletCount2];
	...
	while (true)
	{
		...
		if (true == NewPlayer2.GetIsFire())
		{
			NewBullet2[CurBullet].SetPos(NewPlayer2.GetPos());
			NewBullet2[CurBullet].Fire();
			++CurBullet;
		...
		{
			if (false == NewBullet2[i].GetIsFire())
			{
				continue;
			}
			NewBullet2[i].Move2();
			NewScreen.SetChar(NewBullet2[i]);
		}

		NewScreen.SetChar(NewPlayer);
		NewScreen.SetChar(NewPlayer2);
